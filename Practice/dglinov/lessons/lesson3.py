...
def check_pali(a):
    if isinstance(a, str) and a.lower() == a[::-1].lower():
...
a, b = input("Enter 2 number > 0 to set the range. Format - \'NUM1\',\'NUM2\': ").split(",")
if not a.isdecimal() or not b.isdecimal():
    print("Something wrong! You need to type two numbers > 0 to set the range according to the format!!!")
    exit(1)

secret = randint(min(int(a),int(b)), max(int(a),int(b)))
attempts = 0
while (x:=input(f"Try to guess my number in range {min(a,b)} to {max(a,b)}:\t")).isdecimal():
    attempts += 1
    if int(x) < secret: print(f"My number is bigger than {x}!;)")
    elif int(x) > secret: print(f"My number is smaller than {x}!;)")
    else: 
